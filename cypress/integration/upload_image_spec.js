...
describe('Users should be able to use upload images for use in articles.', () => {
    ...
    it('The user creates a new task', () => {
        ...
        cy.createNewTask('Resize the attached image','Mar 23, 2022','Mar 31, 2022')
    })
    ...
    it('The user uses the image uploaded to the library in the earlier step as the article’s featured image', () => {
        ...
        cy.get('.left-pane > .ndl-Tabs > :nth-child(2) > .ndl-Tabs-option > .ndl-Option-label').click()
        cy.get('.content-tab-header-top > :nth-child(1) > .ndl-Dropdown > .ndl-Button > .ndl-Button-label').click()
        cy.get(':nth-child(4) > .ndl-Dropdown-option > .ndl-Option-label > .ndl-Option-truncatedLabel').click()
        cy.waitFor('ndl-Checkbox-input')
        cy.get('.ndl-Checkbox-label').first().check()
        cy.get('.library-preview-left-buttons-container > .ndl-Button--primary').click()
